...
After running the program, the submitted feedback appears instantly in the Supabase Table Editor. Here is a real example showing data that was inserted by the CLI app:

![Supabase Table Editor showing feedback records](./screenshots/supabase-table.png)

> The `feedback` table displays each entry with an auto-generated `id`, a `created_at` timestamp, the user's `name`, and their `message` — all stored live in the cloud PostgreSQL database.
